...
export default function FeatDescription({ feat, characterAtLevel}: FeatDescriptionProps) {
  ...
  const prerequisitesFormulaText = useMemo(() => feat?.prerequisitesFormulaText, [feat, characterAtLevel]);
  ...
  const isValidForCharacter = useMemo(() => feat.isValidFor(characterAtLevel), [feat, characterAtLevel]);

  return (
      <div>
        {description !== '' && <p><i>{description}</i></p>}
        {prerequisites !== '' && <p><b>Prerequisites:</b> {prerequisites}</p>}
        {/*{!isValidForCharacter && <p><PrerequisiteList featId={feat.id} formulaText={prerequisitesFormulaText} characterAtLevel={characterAtLevel} /></p>}*/}
