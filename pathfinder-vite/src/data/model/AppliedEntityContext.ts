...
import {Choice, FeatureSelectChoice, TextChoice} from "./Choice.ts";
import ChoiceRef from "./ChoiceRef.ts";
import ConditionalComponent from "./ConditionalComponent.ts";
import Entity, {combinePath} from "./Entity.ts";
...
export default class AppliedEntityContext extends BaseDataContext {
  ...
  private applyChoice(basePath: string, choice: Choice, deferred: Deferred): void {
    let path = combinePath(basePath, choice.key);
    if (choice.repeatingIndex > 0) {
      path = path + choice.repeatingIndex;
    }
    this.choices.push(ChoiceRef.create(path, choice, this.database));
    ...
    if (choice instanceof TextChoice && selection) {
      ...
      if (choice.repeatingIndex > 0) {
        this.applyChoice(basePath, choice.repeat(), deferred);
      }
    }
  }
