...
public class ExcelFeatSourceDatabase implements FeatSourceDatabase {
            ...
            try (Workbook workbook = loadClasspathWorkbook("FeatDatabase.xlsx")) {
                var featsByName = StreamSupport.stream(workbook.getSheetAt(0).spliterator(), false)
                        ...
                        .collect(Collectors.toMap(
                                ...
                                feat -> feat,
                                (a, b) -> {
                                    int aSourceIndex = Sources.tryFindSourceByName(a.source()).map(Sources.SOURCES_LIST::indexOf).orElse(Integer.MAX_VALUE);
                                    int bSourceIndex = Sources.tryFindSourceByName(b.source()).map(Sources.SOURCES_LIST::indexOf).orElse(Integer.MAX_VALUE);
    ...
    private static Feat transformToData(Row row) {
        ...
        if (Sources.tryFindSourceByName(source).isEmpty()) {
            return null;
        }
