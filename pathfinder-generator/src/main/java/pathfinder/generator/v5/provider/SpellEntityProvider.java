...
import pathfinder.source.local.SpellSourceDatabase;
...
public class SpellEntityProvider implements EntityProvider {
    ...
    private Entity createSpellEntity(Spell spell) {
        ...
        tags.add(NameToIdConverter.partialId(spell.school()).key);
        spell.levels().forEach(sl -> tags.addAll(Tags.of(
                sl.classId().key,
                sl.classId().key + sl.level())));
        ...
        return Entity.builder()
                ...
                .prerequisiteFormula(prerequisiteFormule(spell.levels()))
                .build();
    }

    private String prerequisiteFormule(List<Spell.Level> spellLevels) {
