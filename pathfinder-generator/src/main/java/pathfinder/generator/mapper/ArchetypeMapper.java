...
public class ArchetypeMapper {
    ...
    private Stream<Feature> mapArchetype(Archetype archetype) {
        ...
        archetype.modifications().forEach(modification -> {
            IdAndLevel toAdd = modification.add();
            Set<IdAndLevel> toRemoveSet = modification.remove();
            if (toRemoveSet == null) {
                toRemoveSet = Set.of();
            }

            pathfinder.model.pathfinder.Feature featureToAdd = archetype.features().stream()
            ...
            if (toAdd.level().isPresent()) {
                ...
                for (var classLevel : baseClass.levels()) {
                    for (IdAndLevel toRemove : toRemoveSet) {
                        boolean levelMatches = toRemove.level().isEmpty() || toRemove.level().getAsInt() == classLevel.level();
