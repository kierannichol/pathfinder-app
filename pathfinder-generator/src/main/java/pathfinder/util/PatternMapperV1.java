...
public class PatternMapperV1 {
    ...
    public PatternMapperV1() {
        replacements = new ArrayList<>();
        patternsByToken = new HashMap<>();
    }

    public PatternMapperV1(PatternMapperV1 copy) {
        replacements = new ArrayList<>(copy.replacements);
        patternsByToken = new HashMap<>(copy.patternsByToken);
    }

    public PatternMapperV1 addToken(String token, String pattern) {
        patternsByToken.put("{" + token.toLowerCase() + "}", Pattern.compile(pattern));
        replacementRegexCache.clear();
        return this;
    }

    public PatternMapperV1 addFunction(String funcKey, BiFunction<String, String, String> func) {
        functionMap.put(funcKey, func);
        return this;
    }

    public PatternMapperV1 addReplacement(String pattern, String replacement) {
        if (!pattern.contains("{")) {
    ...
    public PatternMapperV1 addStaticReplacement(String text, String replacement) {
        staticReplacements.put(text.trim().toLowerCase(), replacement);
        return this;
    }

    public PatternMapperV1 addImmediateReplacement(String pattern, String replacement) {
    ...
    public void addAll(PatternMapperV1 other) {
