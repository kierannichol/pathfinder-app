package pathfinder.parser.db.convert;
...
//            .sorted(Comparator.comparingInt(a -> -a.pattern().pattern().length()))
    ...
    private final List<ReplaceText> featRenames;

    public PrerequisiteConverter(FeatSourceDatabase featSourceDatabase) {
        this.featSourceDatabase = featSourceDatabase;

        this.featRenames = featSourceDatabase.stream()
                .sorted(Comparator.comparingInt(feat -> -feat.name().length()))
                .map(feat -> ReplaceText.map("(^|\\(| AND | OR )%s($|\\)|:|#| AND | OR )".formatted(feat.name()), (before, after) -> "%s@%s%s".formatted(before, feat.id(), after)))
